...
const popularShows = async () => {
  try {
    ...
    const showList = data.results
      .map(
        (item) =>
          `<div class="cards relative border border-amber-400 w-[220px] h-[350px] cursor-pointer">
          <img
          id="cardImg"
          ...
          <div>
            <p class="text-xs font-semibold text-white" >${(item.vote_average).toFixed(1)}/10</p>
          </div>
          </div>
          
          <div class="bio absolute px-2 w-full bottom-0">
            <div class="flex justify-between items-center mb-2">
              <p class="text-white bg-gray-500 inline rounded-lg px-2 py-1 text-xs">${item.original_language}</p>
              <p class="text-white text-xs">${item.first_air_date}</p>
            </div>
            <p class="text-white text-left font-bold text-amber-400">${item.original_name}</p>
          </div>
        </div>`
      )
      .join("");
...
const popularMovies = async () => {
  try {
    ...
    const movieList = data.results
      .map(
        (item) =>
          `<div class="cards relative border w-[220px] h-[350px] cursor-pointer">
          <img
          id="cardImg"
            src="https://image.tmdb.org/t/p/w500/${item.poster_path}"
            alt="${item.original_name}"
          ...
          <div class="bio absolute px-2 w-full bottom-0">
            ...
            <p class="text-white text-left">${item.original_title}</p>
          </div>
        </div>`
      )
      .join("");
